...
#define DEPOP_START_SAMPLES 64 //How many samples to use in dynamic de-popping at the start of a file
#define PAUSE_RAMP_SAMPLES 32 //How many samples to ramp before a pause when pause ramping is enabled
...
SDWavFile::SDWavFile(const char* apFilePath)
{
   ...
   mDepopStart = true;
   mDepopEnd = true;
   mIsPauseRampingEnabled = false;
   mPauseRampDownSamplesLeft = 0;
   mPauseRampDownSamplesLeft = 0;
...
int SDWavFile::Fetch16BitSamples(int16_t* apBuffer, int aNumSamples)
{
         ...
         lSampleIndex++)
   {
      ...
      //Pause ramping
      else if(mIsPauseRampingEnabled)

      {
        float lRampMultiplier = 0.0;
        //Ramp down for pause
        if(mPauseRampDownSamplesLeft > 0)
        {
          lRampMultiplier = 1.0 - (float)mPauseRampDownSamplesLeft/(float)PAUSE_RAMP_SAMPLES;
          mPauseRampDownSamplesLeft--;

          if(mPauseRampDownSamplesLeft <= 0)
          {
            //Done ramping down, so got to full pause mode
            mIsPaused = true;
          }
        }
        //Ramp up for un-pause
        else if(mPauseRampUpSamplesLeft > 0)
        {
          lRampMultiplier = (float)mPauseRampUpSamplesLeft/(float)PAUSE_RAMP_SAMPLES;
          mPauseRampUpSamplesLeft--;
        }

        apBuffer[lSampleIndex] *= lRampMultiplier;
      }
...
{
  if(mIsPauseRampingEnabled && !mIsPaused)
  {
    mPauseRampDownSamplesLeft = PAUSE_RAMP_SAMPLES;
  }
  else
  {
    mIsPaused = true;
...
void SDWavFile::UnPause()
{
  if(mIsPauseRampingEnabled && mIsPaused)
  {
    mPauseRampUpSamplesLeft = PAUSE_RAMP_SAMPLES;
  }
...
void SDWavFile::Skip16BitSamples(int aNumSamples)
{
   ...
   {
      //Read 2 bytes (16 bits) sample from the file
      int8_t laSkippedSample[2]; //This sample will get thrown on the floor, we are skipping it
      mpFileReader->FetchBufferedBytes(laSkippedSample, 2);
...
void SDWavFile::SetPauseRamping(bool aUseRamping)
{
  mIsPauseRampingEnabled = aUseRamping;
}
